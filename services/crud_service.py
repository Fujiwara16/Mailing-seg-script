...
class CrudService:
    def __init__(self, gmail_service, conn):
        ...
        self.conn = conn
        self.email_repo = EmailRepository(conn)
        self.label_repo = LabelRepository(conn)
    ...
    def get_emails_and_store_in_db(self, date_range=3):
        ...
        if emails:
            self.email_repo.batch_insert_emails(emails)
        else:
    ...
    def move_message_to_folder(self, email_id, to_folder, existing_labels):
        ...
        self.email_repo.move_message(email_id, available_labels)
        return email_id

    def mark_as_read(self, message_id):
        self.gmail_service.mark_as_read(message_id)
        self.email_repo.mark_as_read(message_id)
        return message_id

    def mark_as_unread(self, message_id):
        self.gmail_service.mark_as_unread(message_id)
        self.email_repo.mark_as_unread(message_id)
        return message_id
    ...
    def update_labels_mapping(self):
        ...
        for label_name, label_id in labels_dict.items():
            ...
            self.label_repo.insert_label(label_data)
        return labels_dict
